...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Analyze Vision** | Extract requirements, features, assumptions & questions | "Analyze Vision" button | `analyzeProjectAction` / `resumeProjectSynthesisAction` / `createSynthesisGraph` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | `ideaText`, `userAnswers` | Discriminated payload (`COMPLETE` / `NEEDS_INPUT`) | `Project.requirements`, `Project.techStack`, `Project.assumptions`, `Project.openQuestions`, `Project.userAnswers`, `Project.status`, `Feature`, `Decision`, `AuditLog` | Yes (if `NEEDS_INPUT`) | **VERIFIED PRODUCTION** |
| **Architecture Synthesis** | Generate ADRs & system topology | "Generate Architecture" button | `generateArchitectureAction` / `architectureSynthesisNode` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | `projectName`, `ideaText`, `techStack` | `SystemArchitectureSynthesisResult` | `Project.architecture`, `Decision` | No | **VERIFIED PRODUCTION** |
| **Roadmap Synthesis** | Generate milestone schedule | "Generate Roadmap" button | `generateRoadmapAction` / `roadmapSynthesisNode` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | `projectName`, `features`, `decisions` | `RoadmapSynthesisResult` | `RoadmapItem`, `Project.status` | No | **VERIFIED PRODUCTION** |
| **Document Generation** | Generate Markdown spec docs | "Generate Specification" button | `generateDocumentAction` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | `docType`, live project state graph | Markdown text | `Document` (version incremented) | No | **VERIFIED PRODUCTION** |
| **UI Design System** | Synthesize DesignSpec & preview | "Generate AI Design System" | `generateUiDesignAction` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | Markdown spec / Design presets | Validated `DesignSpec` JSON | `Project.architecture` | No | **VERIFIED PRODUCTION** |
| **Agent Chat** | Context-grounded AI copilot | Workspace Chat Drawer | `sendChatMessageAction` / `getChatHistoryAction` | Groq ➔ Gemini | `openai/gpt-oss-120b` / `gemini-2.5-flash` | User message + live state graph | Markdown reply + proposal JSON | `ChatMessage`, `ChatSession` | No | **VERIFIED PRODUCTION** |
| **Accept Proposal** | Mutate project state from chat | "Accept Proposal" button | `acceptProposalAction` | N/A (Server Action) | N/A | `ProposalPayload` | Updated project field | `Project.*`, `Decision`, `AuditLog` | Yes | **VERIFIED PRODUCTION** |
| **Tavily Web Search** | Live tech stack research | Auto in chat / docs | `searchTavily` | Tavily API | `tavily-search` | Query string | Search snippets | `AiUsageLog` | No | **VERIFIED PRODUCTION** |
